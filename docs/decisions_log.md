...
**Decision:** Club Hamilton has 59 assets across 5 tiers with the following display names used in the POS and asset board:

| Asset Type | Count | Display Name |
|---|---|---|
| Locker | 33 | Lckr |
| Room | 11 | Sing STD |
| Room | 10 | Sing DLX |
...
**Rationale:** Display names are abbreviated for readability on the POS screen and asset board. Full names are: Locker, Single Standard Room, Single Deluxe Room, Glory Hole Room, Double Deluxe Room.
...
**Date:** 2026-04-08  
**Context:** The asset board needs overtime thresholds to display visual warnings when a guest has exceeded their expected stay.  
**Decision:** Expected stay duration is 6 hours for all asset types — both rooms and lockers.  
**Rationale:** Hamilton operates a uniform stay policy across all asset categories. The 6-hour threshold triggers the overtime indicator on the asset board regardless of asset type or tier.

---

## DEC-010 — Shift Float Amount

**Date:** 2026-04-08  
**Context:** Operators start each shift with a cash float. The amount needed to be defined and its configurability clarified.  
**Decision:** The default float amount for Club Hamilton is $200. The float amount is configurable per venue via a system setting — it is not hardcoded.  
**Rationale:** Hamilton's current operational standard is $200. Other venues (Philadelphia, Dallas, Toronto) may use different amounts. Storing this as a configurable setting avoids code changes when the amount is adjusted.
...
**Date:** 2026-04-08  
**Context:** Cash drop envelopes require a printed label. Printer model and driver approach were TBD.  
**Decision:** Brother QL-820NWB, connected via WiFi. Printing is handled via the Brother network API called from the Frappe backend.  
**Rationale:** The Brother QL-820NWB has reliable Linux support, which is required for the Frappe Cloud hosting environment. Network (WiFi) connectivity means printing is not dependent on which tablet is physically connected. Backend API printing is more reliable than browser-based print dialogs. The printer IP address is configurable via a system setting — not hardcoded — so it can be changed without code modifications.
