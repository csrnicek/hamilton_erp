...
## R-011 — Cash Reconciliation variance system non-functional at Phase 1 (false-alarm risk) — **CLOSED (deferred to Phase 3 via DEC-069)**

**Status as of 2026-05-03:** Closed. Path B shipped in DEC-069 — `_set_variance_flag` short-circuits to `"Pending Phase 3"` regardless of inputs, so the false-positive flag pattern can no longer occur. Managers follow the manual reconciliation procedure documented in `HAMILTON_LAUNCH_PLAYBOOK.md` #3 and `RUNBOOK.md` §7.2 until Phase 3 wires up the real `system_expected` calculation. The original risk write-up below is preserved for context.
...
- **Mitigation in place (superseded by DEC-069):** ~~Manager training. Until Phase 3 ships, managers must ignore the `variance_flag` value entirely and treat the count as Clean if A (operator-declared) ≈ C (manager-counted) and the printed envelope label matches the operator declaration. The flag is noise, not signal, while system_expected is stubbed.~~ **2026-05-03:** Replaced by code — Path B short-circuits the classifier so the flag reads `"Pending Phase 3"` regardless of inputs. No training reliance.
