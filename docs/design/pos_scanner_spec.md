...
1. **ID acceptance — driver's license is the scanner-supported path; the system accepts other government photo IDs via manual entry.**
   - **Scanner path (PDF417 DL parsing):** must reliably parse Canadian provincial DLs (Ontario, Quebec, BC, Alberta — expected at Hamilton) and US state DLs (anywhere — expected at Philadelphia / DC / Dallas). Parsed payload includes DOB, name, expiry, ID number per AAMVA spec.
   - **Customers may present non-DL government photo IDs**: passport, provincial / state age-of-majority card, military ID, PR card, citizenship card, or other official government photo ID. These are NOT machine-readable via the scanner — they have no PDF417 payload, or use a different format (e.g. passport MRZ, which is a separate spec). The scanner does not need to parse these.
   - **System workflow fallback:** the Hamilton ERP check-in flow MUST accept manual entry of DOB / name / ID number for customers presenting non-DL government photo IDs. The operator visually verifies the ID, types the relevant fields, and the check-in proceeds with the same audit trail as a scanned DL. The audit log records `id_method = "manual"` (vs `id_method = "scanned_dl"`) so back-office reporting can distinguish.
   - **Out of scope for THIS spec** (scanner hardware): the broader ID-acceptance UX flow, manual-entry form design, and ID-type taxonomy. Those land in the Phase 2 check-in flow design. The scanner spec just confirms the scanner doesn't BLOCK non-DL IDs — operators can always bypass it.
