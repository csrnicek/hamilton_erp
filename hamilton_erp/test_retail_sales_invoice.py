...
class TestSubmitRetailSale(IntegrationTestCase):
	...
	def test_submit_applies_hst_tax_line_at_13_percent(self):
		...
		si = frappe.get_doc("Sales Invoice", result["sales_invoice"])
		self.assertEqual(si.net_total, 10.00)
		...
		self.assertAlmostEqual(float(hst.tax_amount), 1.30, places=2)
		self.assertAlmostEqual(float(si.grand_total), 11.30, places=2)
	...
	def test_submit_returns_correct_change(self):
		...
		# grand_total = 10 + 1.30 HST = 11.30; change = 20 - 11.30 = 8.70
		self.assertAlmostEqual(result["grand_total"], 11.30, places=2)
		self.assertAlmostEqual(result["change"], 8.70, places=2)
