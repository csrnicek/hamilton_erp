...
class TestFamilyE_BulkOperationAttacks(IntegrationTestCase):
	"""Attacks against bulk clean operations (DEC-054).

	Tests that bulk clean correctly transitions Dirty → Available,
	skips Occupied and OOS assets, and isolates per-asset errors.
	"""

	def setUp(self):
		_ensure_walkin_customer()
		# Create 3 Room assets in different states for bulk testing
		self.dirty1 = _make_asset(prefix="E-D1", display_order=9800)
		self.dirty2 = _make_asset(prefix="E-D2", display_order=9801)
		self.occupied = _make_asset(prefix="E-OCC", display_order=9802)
		self.oos = _make_asset(prefix="E-OOS", display_order=9803)

		# Walk dirty assets to Dirty state
		_walk_to_dirty(self.dirty1.name)
		_walk_to_dirty(self.dirty2.name)
		# Walk occupied asset to Occupied
		_walk_to_occupied(self.occupied.name)
		# Walk OOS asset to Out of Service
		_walk_to_oos(self.oos.name)

	def tearDown(self):
		frappe.db.rollback()

	# E01 — Bulk clean transitions all dirty rooms to Available
	def test_e01_bulk_clean_dirty_rooms(self):
		attack_id = "E01"
		try:
			lifecycle.mark_asset_clean(
				self.dirty1.name, operator="Administrator"
			)
			lifecycle.mark_asset_clean(
				self.dirty2.name, operator="Administrator"
			)
			a1 = frappe.get_doc("Venue Asset", self.dirty1.name)
			a2 = frappe.get_doc("Venue Asset", self.dirty2.name)
			self.assertEqual(a1.status, "Available")
			self.assertEqual(a2.status, "Available")
			_log_pass(attack_id)
		except Exception as e:
			_log_crash(attack_id, e)
			raise

	# E02 — Bulk clean does NOT affect Occupied assets
	def test_e02_bulk_clean_skips_occupied(self):
		attack_id = "E02"
		try:
			# Clean the dirty assets
			lifecycle.mark_asset_clean(
				self.dirty1.name, operator="Administrator"
			)
			# Verify Occupied asset is unchanged
			asset = frappe.get_doc("Venue Asset", self.occupied.name)
			self.assertEqual(asset.status, "Occupied",
				"Occupied assets must not be affected by cleaning other assets")
			_log_pass(attack_id)
		except Exception as e:
			_log_crash(attack_id, e)
			raise

	# E03 — Bulk clean does NOT affect OOS assets
	def test_e03_bulk_clean_skips_oos(self):
		attack_id = "E03"
		try:
			lifecycle.mark_asset_clean(
				self.dirty1.name, operator="Administrator"
			)
			asset = frappe.get_doc("Venue Asset", self.oos.name)
			self.assertEqual(asset.status, "Out of Service",
				"OOS assets must not be affected by cleaning other assets")
			_log_pass(attack_id)
		except Exception as e:
			_log_crash(attack_id, e)
			raise

	# E04 — _mark_all_clean via the API layer (permission + full path)
	def test_e04_mark_all_clean_rooms_api(self):
		attack_id = "E04"
		try:
			...
			self.assertIsInstance(result["succeeded"], list)
			self.assertIsInstance(result["failed"], list)
			# Our dirty rooms should have been cleaned
			...
			# Occupied and OOS still unchanged
			occ = frappe.get_doc("Venue Asset", self.occupied.name)
			oos = frappe.get_doc("Venue Asset", self.oos.name)
			self.assertEqual(occ.status, "Occupied")
			self.assertEqual(oos.status, "Out of Service")
			_log_pass(attack_id)
		except Exception as e:
			_log_crash(attack_id, e)
			raise

	# E05 — Bulk clean on Locker category (separate from Room)
	def test_e05_bulk_clean_lockers_separate(self):
		attack_id = "E05"
		try:
			# Create a dirty Locker
			locker = _make_asset(
				prefix="E-LK", category="Locker",
				tier="Locker", display_order=9810
			)
			_walk_to_dirty(locker.name)

			from hamilton_erp.api import _mark_all_clean

			result = _mark_all_clean(category="Locker")
			self.assertGreaterEqual(len(result["succeeded"]), 1)

			lk = frappe.get_doc("Venue Asset", locker.name)
			self.assertEqual(lk.status, "Available")

			# Dirty ROOM assets must NOT have been cleaned by Locker bulk
			d1 = frappe.get_doc("Venue Asset", self.dirty1.name)
			self.assertEqual(d1.status, "Dirty",
				"Room assets must not be affected by Locker bulk clean")
			_log_pass(attack_id)
		except Exception as e:
			_log_crash(attack_id, e)
			raise
...
# ===========================================================================
# FAMILY F — Financial & Phase 2 Attacks (all skipped)
# ===========================================================================
...
def load_tests(loader, tests, pattern):
	...
	test_classes = [
		...
		TestFamilyE_BulkOperationAttacks,
