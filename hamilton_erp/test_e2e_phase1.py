...
def real_logs():
	...
	`_make_asset_status_log` reads `frappe.in_test` (the module attribute
	at frappe/__init__.py:83), which is set by Frappe's test runner via
	`frappe.tests.utils.toggle_test_mode`. It is NOT aliased to
	`frappe.flags.in_test` — they are independent variables. E2E tests
	that need real audit logs must clear `frappe.in_test` directly.
	"""
